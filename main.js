        ...
        if (err) {
            ...
            fs.writeFile(des_file, data, function (err) {
                if (err) {
                    ...
                    connection.query(imginsert,imgParam,  function (err, result) {
                        if (err) {
                            console.log(err);
                        }
                    })
...
app.use('/users', function (request, response) {
    if (request.body.code == '1001') {//用户注册
        ...
        connection.query(CheckAddSql, function (err, result) {
        ...
        if (request.session.studentisadmin == 0) {
            ...
            connection.query(selectifresume, function (err, result) {
                if (err) {
                    console.log(' error- ' + err.message)
                    response.end('error')
                    return
                    ...
                    connection.query(selectjoblist, function (err, result) {
                        if (err) {
                            console.log(' error- ' + err.message)
                            response.end('error')
                            return
                            ...
                            connection.query(insertjoblist, insertjoblistParams, function (err, result) {
                                if (err) {
                                    console.log(' error- ' + err.message)
                                    response.end('error')
                                    return
        ...
        connection.query(checkresumesql, function (err, result) {
            console.log(result.length)
            if (err) {
                log.err('[error]-' + error)
                response.end('error')
                ...
                connection.query(addResumesql, addResumeParam, function (err, result) {
                    if (err) {
                        response.end("error")
                        console.log("error-" + err.message)
                        return
        ...
        connection.query(checkuser, function (err, result) {
            if (err) {
                ...
                connection.query(selectpasswd, function (err, result) {
                    if (err) {
                        log.err('[error]-' + error)
                        response.end('error')
                        return
...
app.use('/bussiness', function (request, response) {
    if (request.body.code == '1001') {//发布工作
        ...
        var addjobParam = [request.body.companyname, request.body.worktype, request.body.type,  request.body.place, request.body.company, request.body.money, request.body.edubackground, request.body.workyear, request.body.email, request.body.phone, request.body.detail2, request.body.detail3,request.body.welfare,request.body.contact]
        connection.query(addjob, addjobParam, function (err, result) {
            if (err) {
                response.end("error")
                console.log("error-" + err.message)
                return
    ...
    if (request.body.code == "1002") {//用户企业留言
        ...
        connection.query(addadvice, addadviceParam, function (err, result) {
            if (err) {
                response.end("error")
                console.log("error-" + err.message)
                return
    ...
    if (request.body.code == "1003") {//企业描述
        ...
        connection.query(selectjob, request.body.name, function (err, result) {
            if (err) {
                response.end("error")
                console.log("error-" + err.message)
                return
    ...
    if (request.body.code == "1005") {//应聘者列表
        ...
        connection.query(selectuserlist, request.body.companyname, function (err, result) {
            if (err) {
                response.send("error")
                console.log("error-" + err.message)
                return
                ...
                connection.query(selectuserinfo, function (err, result) {
                    if (err) {
                        console.log("error" + err.message)
                        response.send("error")
                        return
                    ...
                    if (result.length == 0) {
                        ...
                        connection.query(addSql, addSqlParams, function (err, result) {
                            if (err) {
                                ...
                                connection.query(adduserssql, adduserssqlParams, function (err, result) {
                                    if (err) {
                                        console.log(' error- ' + err.message)
                                        response.end('error')
                                        return
        ...
        connection.query(selectjobnum, function (err, result) {
            if (err) {
                response.end("error")
                console.log("error-" + err.message)
            }
        ...
        connection.query(selectcompanynum, function (err, result) {
            if (err) {
                response.end("error")
                console.log("error-" + err.message)
            }
...
app.use('/center', function (request, response) {
    ...
    if (request.body.code == "1001") {//个人中心
        ...
        connection.query(selectstuInfo, function (err, result) {
            if (err) {
                response.send("error")
                console.log("error-" + err.message)
                return
        ...
        connection.query(resumelist, function (err, result) {
            if (err) {
                response.send("error")
                console.log("error-" + err.message)
                return
        ...
        if (request.body.name == null) {
            ...
            connection.query(resumeInfo, function (err, result) {
                if (err) {
                    response.send("error")
                    console.log("error-" + err.message)
                    return;
            ...
            connection.query(Resumesql, ResumeParam, function (err, result) {
                if (err) {
                    response.end("error")
                    console.log("error-" + err.message)
                    return
        ...
        if (request.body.action == "reject") {
            ...
            connection.query(updateResumelist, function (err, result) {
                if (err) {
                    response.send("error")
                    console.log("error-" + err.message)
                    return
            ...
            connection.query(companyInfo, function (err, result) {
                if (err) {
                    response.send("error")
                    console.log("error-" + err.message)
                    return
